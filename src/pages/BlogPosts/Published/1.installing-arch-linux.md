...
Next, I installed `networkmanager` with `pacman -S networkmanager` and enabled
it with `systemctl enable NetworkManager.service`.

With that, the network on the system is setup and now I need to create a
password and install a bootloader.
